package com.xingguang.sinanya.db.roles;
...
import static com.xingguang.sinanya.tools.getinfo.RoleInfo.getRoleInfoByQQ;
...
public class InsertRoles {
    private static final Logger log = LoggerFactory.getLogger(InsertRoles.class.getName());
    ...
    public void insertRoleChoose(long qqId, String role) {
        ...
        try (Connection conn = DbUtil.getConnection()) {
            //language=MySQL
            String sql = "select * from test.CHOOSE_ROLE where qq=?";
            ...
            if (num == 0) {
                sql = "INSERT INTO test.CHOOSE_ROLE(" +
                        "qq," +
                        "role" +
                        ") VALUES(?,?)";
                try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ...
                sql = "update test.CHOOSE_ROLE set " +
                        "role=? where qq=?";
                try (PreparedStatement ps = conn.prepareStatement(sql)) {
                    ps.setString(1, role);
                    ps.setLong(2, qqId);
    ...
    public void insertRoleInfo(String properties, String role, long qqId) {
        HashMap<String, Integer> propertiesForRole = (HashMap<String, Integer>) RoleInfo.getRoleInfoByQQ(qqId, role);
    ...
    public void insertRoleInfo(HashMap<String, Integer> properties, String role, long qqId) {
        ...
        try (Connection conn = DbUtil.getConnection()) {
            //language=MySQL
            String sql = "select * from test.role where userName=? and qqId=?";
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ps.setString(1, role);
                ps.setLong(2, qqId);
                try (ResultSet set = ps.executeQuery()) {
        ...
        RoleInfoCache.ROLE_INFO_CACHE.put(new EntityRoleTag(qqId, role), properties);
        if (num == 0) {
    ...
    private void updateInfo(HashMap<String, Integer> propertiesForRole, long qqId, String role) {
        ...
        if (propertiesForRole == null || propertiesForRole.size() <= 113) {
            return;
        }
        try (Connection conn = DbUtil.getConnection()) {
            //language=MySQL
            String sql = "update test.role set createTime=?,  str=?, dex=?, pow=?, con=?, app=?, edu=?, siz=?, intValue=?, san=?, luck=?, mp=?, hp=?, accounting=?, anthropology=?, evaluation=?, archaeology=?, enchantment=?, toClimb=?, computerUsage=?, creditRating=?, cthulhuMythos=?, disguise=?, dodge=?, drive=?, electricalMaintenance=?, electronics=?, talkingSkill=?, aFistFight=?, wrangle=?, pistol=?, firstAid=?, history=?, intimidate=?, jump=?, motherTongue=?, law=?, libraryUse=?, listen=?, `unlock`=?, mechanicalMaintenance=?, medicalScience=?, naturalHistory=?, naturalScience=?, pilotage=?, occultScience=?, operatingHeavyMachinery=?, persuade=?, psychoanalysis=?, psychology=?, horsemanship=?, aWonderfulHand=?, investigationOfCrimes=?, stealth=?, existence=?, swimming=?, throwValue=?, trackValue=?, domesticatedAnimal=?, diving=?, blast=?, lipReading=?, hypnosis=?, artillery=?,shipping=?, rifle=?, aircraftOperation=?, machineGun=?, sing=?, paint=?, tillage=?, photography=?, perform=?, forge=?, literature=?, calligraphy=?, music=?, cooking=?, tailor=?, haircut=?, architecture=?, dance=?, makeWine=?, fishing=?, potteryMaking=?, sculpture=?, acrobatics=?, fengshui=?, technicalDrawing=?, typing=?, shorthand=?, whip=?, electricSaw=?, axe=?, sword=?, flail=?, spear=?, submachineGun=?, archery=?, flameEjector=?, heavyWeapons=?, ride=?, geology=?, chemistry=?, biology=?, mathematics=?, astronomy=?, physics=?, pharmacy=?, botany=?, zoology=?, cryptography=?, engineering=?, meteorology=?, judicialScience=?,other=? where userName=? and qqId=? ";
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ...
                ps.setString(116, makeOtherSkill(propertiesForRole));
                ps.setString(117, role);
                ps.setLong(118, qqId);
    ...
    private void insertInfo(HashMap<String, Integer> propertiesForRole, long qqId, String role) {
        ...
        try (Connection conn = DbUtil.getConnection()) {
            //language=MySQL
            String sql = "INSERT INTO test.role(createTime,qqId,userName,str,dex,pow,con,app,edu,siz,intValue,san,luck,mp,hp,accounting,anthropology,evaluation,archaeology,enchantment,toClimb,computerUsage,creditRating,cthulhuMythos,disguise,dodge,drive,electricalMaintenance,electronics,talkingSkill,aFistFight,wrangle,pistol,firstAid,history,intimidate,jump,motherTongue,law,libraryUse,listen,`unlock`,mechanicalMaintenance,medicalScience,naturalHistory,naturalScience,pilotage,occultScience,operatingHeavyMachinery,persuade,psychoanalysis,psychology,horsemanship,aWonderfulHand,investigationOfCrimes,stealth,existence,swimming,throwValue,trackValue,domesticatedAnimal,diving,blast,lipReading,hypnosis,artillery,shipping,rifle,aircraftOperation,machineGun,sing,paint,tillage,photography,perform,forge,literature,calligraphy,music,cooking,tailor,haircut,architecture,dance,makeWine,fishing,potteryMaking,sculpture,acrobatics,fengshui,technicalDrawing,typing,shorthand,whip,electricSaw,axe,sword,flail,spear,submachineGun,archery,flameEjector,heavyWeapons,ride,geology,chemistry,biology,mathematics,astronomy,physics,pharmacy,botany,zoology,cryptography,engineering,meteorology,judicialScience,other) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ps.setTimestamp(1, timestamp);
                ps.setLong(2, qqId);
                ps.setString(3, role);
                ...
                String other = makeOtherSkill(propertiesForRole);
                ps.setString(118, other);
                ps.executeUpdate();
            }
        } catch (SQLException e) {
            log.error(e.getMessage(), e);
    ...
    public void deleteInfo(long qqId, String role) {
        try (Connection conn = DbUtil.getConnection()) {
            //language=MySQL
            String sql = "delete from test.role where " +
                    "qqId=? and " +
                    "userName=?";
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ps.setLong(1, qqId);
    ...
    public String makeOtherSkill(HashMap<String, Integer> propertiesForRole) {
        StringBuilder stringBuilder = new StringBuilder();
        Map<String, Integer> init = new MakeRolesInfo().init();
        for (Map.Entry<String, Integer> everyProperties : propertiesForRole.entrySet()) {
            if (!init.containsKey(everyProperties.getKey())) {
                stringBuilder.append(everyProperties.getKey().trim()).append(everyProperties.getValue());
            }
        }
        return stringBuilder.toString();
    }

    public void insertRoleTmpName(long qqId,String userName){
        try (Connection conn = DbUtil.getConnection()) {
            //language=MySQL
            String sql = "replace into test.roleTmpName set qqId=?,userName=?";
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ps.setLong(1, qqId);
    ...
    public void deleteRoleTmpName(long qqId){
        try (Connection conn = DbUtil.getConnection()) {
            //language=MySQL
            String sql = "delete from test.roleTmpName where qqId=?";
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ps.setLong(1, qqId);
                ps.executeUpdate();
            }
