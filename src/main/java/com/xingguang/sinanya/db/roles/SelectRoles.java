...
public class SelectRoles {
    private static final Logger log = LoggerFactory.getLogger(SelectRoles.class.getName());
    ...
    public static void flushRoleInfoCacheFromDatabase() {
        RoleInfoCache.ROLE_NAME_CACHE.clear();
        try (Connection conn = DbUtil.getConnection()) {
            //language=MySQL
            String sql = "select userName,qqId from test.role";
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                try (ResultSet set = ps.executeQuery()) {
                    while (set.next()) {
                        RoleInfoCache.ROLE_NAME_CACHE.add(new EntityRoleTag(Long.parseLong(set.getString("qqId")), set.getString("userName")));
                    }
                }
            }
    ...
    public static void flushRoleTmpNameFromDatabase(){
        RoleInfoCache.ROLE_TMP_NAME.clear();
        try (Connection conn = DbUtil.getConnection()) {
            //language=MySQL
            String sql = "select userName,qqId from test.roleTmpName";
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                try (ResultSet set = ps.executeQuery()) {
                    while (set.next()) {
                        RoleInfoCache.ROLE_TMP_NAME.put(Long.parseLong(set.getString("qqId")), set.getString("userName"));
                    }
                }
            }
        } catch (SQLException e) {
    ...
    public static void selectRoleInfoCacheFromDatabase(String role, String qqId) {
        try (Connection conn = DbUtil.getConnection()) {
            //language=MySQL
            ...
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ...
                try (ResultSet set = ps.executeQuery()) {
                    while (set.next()) {
                        ...
                        if (set.getString("other") != null && !set.getString("other").equals("")) {
                            propertiesForRole = (HashMap<String, Integer>) new MakeRolesInfo(set.getString("other"), propertiesForRole).getPropertiesForRole();
                        }
                        RoleInfoCache.ROLE_INFO_CACHE.put(new EntityRoleTag(Long.parseLong(qqId), role), propertiesForRole);
                    }
                }
            }
        } catch (SQLException e) {
    ...
    public void flushRoleChooseFromDatabase() {
        try (Connection conn = DbUtil.getConnection()) {
            //language=MySQL
            String sql = "select * from test.CHOOSE_ROLE";
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                try (ResultSet set = ps.executeQuery()) {
                    while (set.next()) {
                        RoleInfoCache.ROLE_CHOOSE.put(set.getLong("qq"), set.getString("role"));
                    }
                }
            }
        } catch (SQLException e) {
