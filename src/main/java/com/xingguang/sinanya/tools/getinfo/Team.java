...
import com.xingguang.sinanya.db.team.InsertTeam;
import com.xingguang.sinanya.db.team.SelectTeam;
...
public class Team {
    ...
    /**
     * 保存成员幕间成长标记数据，这个方法会在com.xingguang.sinanya.listener.InputHistoryToDataBase中定时调用
     */
    public static void saveTeamEn() {
        for (Map.Entry<EntityQqAndGroup, ArrayList<String>> mapEntry : MessagesTeamEn.TEAM_EN.entrySet()) {
    ...
    public static String getTeamEn(EntityTypeMessages entityTypeMessages, String qqId) {
        EntityQqAndGroup entityQqAndGroup = new EntityQqAndGroup(entityTypeMessages.getFromGroupString(), qqId);
        if (MessagesTeamEn.TEAM_EN.containsKey(entityQqAndGroup)) {
            ...
            return role + StringUtils.join(MessagesTeamEn.TEAM_EN.get(entityQqAndGroup), ",");
        } else {
