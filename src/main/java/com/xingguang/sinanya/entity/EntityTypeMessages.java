...
import static com.xingguang.sinanya.tools.checkdata.CheckValue.checkValue;
...
public class EntityTypeMessages {
    ...
    public EntityTypeMessages(MsgGetTypes msgGetTypes, MsgSender msgSender, MsgGet msgGet,GroupMsg msgGroup,PrivateMsg msgPrivate, DiscussMsg msgDisGroup) {
        this.msgGetTypes = msgGetTypes;
        this.msgSender = msgSender;
        this.msgGet = msgGet;
        this.msgDisGroup = msgDisGroup;
    ...
    public long getFromQq() {
        switch (msgGetTypes) {
            case groupMsg:
                return Long.parseLong(msgGroup.getQQ());
            case discussMsg:
                return Long.parseLong(msgDisGroup.getQQCode());
            case privateMsg:
                return Long.parseLong(msgPrivate.getQQCode());
            default:
                return 450609203;
        }
    }
    ...
    public long getFromGroup() {
        switch (msgGetTypes) {
            case groupMsg:
                return Long.parseLong(msgGroup.getGroupCode());
            case discussMsg:
                return Long.parseLong(msgDisGroup.getGroupCode());
            default:
                return 0L;
        }
    }
     ...
     * @return 获取来源群号，需要根据消息来源，分别从2中不同渠道取得，这里我默认写了0
     */
    public String getFromGroupString() {
        switch (msgGetTypes) {
            case groupMsg:
