...
public interface GetRandomList {
    Logger log = LoggerFactory.getLogger(GetRandomList.class.getName());
    ...
    default String randomFromList(ArrayList<String> infoList) {
        String tmp = "";
        int index = RandomInt.random(0, infoList.size() - 1);
        try {
    ...
    default String randomFromListSmall(ArrayList<String> infoList) {
        ...
        return infoList.get(RandomInt.random(0, min(1, (infoList.size() - random))));
    }
    ...
    default String randomNestList(HashMap<Integer, ArrayList<String>> infoList) {
        int random = RandomInt.random(0, infoList.size() - 1);
        ArrayList<String> detail = infoList.get(random);
        return detail.get(RandomInt.random(0, detail.size() - 1));
    }
}
